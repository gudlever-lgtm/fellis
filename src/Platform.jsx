...
function BillingSettings({ lang, t }) {
  ...
  useEffect(() => {
    ...
    if (params.get('adfree') === 'success') {
      ...
      url.searchParams.delete('adfree')
  ...
  return (
    <div>
      {verifying && (
        <div style={{ padding: '10px 16px', background: '#fff8e1', border: '1px solid #ffe082', borderRadius: 10, marginBottom: 12, fontSize: 13, color: '#7a5800' }}>
          {lang === 'da' ? 'Verificerer betaling…' : 'Verifying payment…'}
        </div>
      )}
      {verifyError && (
        <div style={{ padding: '10px 16px', background: '#fff3f3', border: '1px solid #f5c6cb', borderRadius: 10, marginBottom: 12, fontSize: 13, color: '#721c24' }}>
          {verifyError}
        </div>
      )}
            ...
            {!showCancelConfirm && isCancelled && !isModeChange && (
              <button
                onClick={async () => {
                  ...
                  const r = await apiReactivateAdfree().catch(() => null)
                  setReactivating(false)
                  if (r?.ok) loadSub()
                  else setCancelError(lang === 'da' ? 'Kunne ikke genaktivere abonnementet.' : 'Could not reactivate subscription.')
                }}
                disabled={reactivating}
                style={{ background: '#2D6A4F', border: 'none', borderRadius: 8, padding: '8px 16px', fontSize: 13, color: '#fff', fontWeight: 700, cursor: reactivating ? 'not-allowed' : 'pointer', opacity: reactivating ? 0.7 : 1 }}
              >
                {reactivating ? (lang === 'da' ? 'Genaktiverer…' : 'Reactivating…') : t.adFreeReactivateBtn}
              </button>
            )}
...
function AdsManagementPage({ lang, t }) {
  ...
  useEffect(() => {
    ...
    if (params.get('ad_payment') === 'success') {
      ...
      url.searchParams.delete('session_id')
      window.history.replaceState({}, '', url)
      // Verify payment with Stripe and activate the ad
      if (adId && sessionId) {
        apiVerifyAdPayment(adId, sessionId).then(r => {
          if (r?.ok) { setSuccessBanner(true); setTimeout(() => setSuccessBanner(false), 6000); reload() }
        }).catch(() => {})
      } else {
        // Webhook may have already handled it — just reload
