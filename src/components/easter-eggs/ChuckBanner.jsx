...
const FALLBACK = "Chuck Norris can divide by zero."

/**
 * ChuckBanner — overlays the bottom status bar with a Chuck Norris joke.
 * Fetches from chucknorris.io; falls back to hardcoded quote on error.
...
export default function ChuckBanner({ onDismiss }) {
  ...
  useEffect(() => {
    ...
    fetch('https://api.chucknorris.io/jokes/random')
      ...
      .then(d => { if (!cancelled) setJoke(d?.value || FALLBACK) })
      .catch(() => { if (!cancelled) setJoke(FALLBACK) })
  ...
  return (
    <div
      ...
      style={{
        ...
        boxShadow: '0 -2px 12px rgba(192,104,26,0.2)',
      }}
    >
      ...
      <span style={{ fontSize: 13, color: '#bbb', flexShrink: 0 }}>✕ Esc</span>
    </div>
  )
}
