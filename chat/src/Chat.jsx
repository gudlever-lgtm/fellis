...
function CameraModal({ lang, onCapture, onClose }) {
  ...
  const s = {
    ...
    btnCapture: { padding: '12px 28px', borderRadius: 8, background: ready ? '#1877f2' : '#555', color: '#fff', border: 'none', cursor: ready ? 'pointer' : 'default', fontSize: 15, fontWeight: 700 },
    ...
    <div style={s.overlay} onClick={e => e.target === e.currentTarget && onClose()}>
      {error ? (
        <div style={s.errorText}>
          ...
          <button onClick={onClose} style={s.btnCancel}>{t(lang, 'close')}</button>
        </div>
      ) : (
        <>
          <video ref={videoRef} autoPlay playsInline muted style={s.video} />
          <div style={s.row}>
            <button onClick={onClose} style={s.btnCancel}>{t(lang, 'cancel')}</button>
            <button onClick={() => { streamRef.current?.getTracks().forEach(tr => tr.stop()); setReady(false); setError(null); setFacing(f => f === 'environment' ? 'user' : 'environment') }} style={s.btnFlip}>🔄</button>
            <button onClick={capture} style={s.btnCapture} disabled={!ready}>{t(lang, 'capture')}</button>
          </div>
        </>
      )}
    </div>
